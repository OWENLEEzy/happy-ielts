...
export default function GeneralOnboardingPage() {
  ...
  const startOnboarding = async () => {
    ...
    setMessages([{
      role: 'assistant',
      content: `你好！我会帮你规划关于「${topic}」的学习课程。先告诉我，你学习它最想实现什么目标？`,
    }])
  }
  ...
  if (!started) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-4"
        ...
        <h1 className="text-4xl mb-2 font-semibold" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#c9a84c' }}>
          Scholar&apos;s Atelier
        </h1>
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: '#0f0d1a', color: '#e8e0d0' }}>
      {/* Header */}
      <div className="flex items-center gap-3 px-6 py-4 border-b" style={{ borderColor: '#2e2850' }}>
        <span className="text-lg font-semibold" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#c9a84c' }}>
          学习规划
        </span>
      ...
      {preview && (
        <div className="px-4 pb-4 max-w-2xl w-full mx-auto">
          <div className="rounded-xl p-4 text-sm" style={{ background: '#1a1730', border: '1px solid #c9a84c33' }}>
            <p className="font-medium mb-2" style={{ color: '#c9a84c' }}>📋 学习计划草稿</p>
      ...
      {!preview && (
        <div className="px-4 pb-6 pt-2 max-w-2xl w-full mx-auto flex gap-2">
          <textarea
            ...
            style={{ background: '#1a1730', border: '1px solid #2e2850', color: '#e8e0d0', minHeight: 48 }}
            rows={1}
            placeholder="回复顾问..."
            value={input}
