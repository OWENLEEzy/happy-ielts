...
export default function LessonPage() {
  ...
  if (phase === 'done') {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-4"
        ...
        <h2 className="text-2xl font-semibold mb-2" style={{ color: s.amber }}>课程完成</h2>
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: s.bg, color: s.fg }}>
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: s.border }}>
        ...
        <span className="text-sm px-3 py-1 rounded-full capitalize"
          ...
          {phase === 'reading' ? '阅读' : phase === 'quiz' ? '测验' : phase === 'free_qa' ? '问答' : phase}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 max-w-2xl w-full mx-auto">
        ...
        {phase === 'reading' && (
          <div>
            <div
              ...
              style={{ background: s.card, border: `1px solid ${s.border}` }}
            >
              {studyGuide || <span className="opacity-40 animate-pulse">正在加载学习材料...</span>}
            </div>
            <button
        ...
        {phase === 'quiz' && (
          <div>
            <h3 className="text-lg font-semibold mb-4" style={{ color: s.amber }}>随堂测验</h3>
            <div className="flex flex-col gap-4 mb-6">
              {quiz.map((q, i) => (
                <div key={i} className="rounded-xl p-4" style={{ background: s.card, border: `1px solid ${s.border}` }}>
                  <p className="text-sm mb-3 font-medium">{i + 1}. {q.q}</p>
                  <input
        ...
        {phase === 'free_qa' && (
          <div>
            <h3 className="text-lg font-semibold mb-4" style={{ color: s.amber }}>自由问答</h3>
            <div className="flex flex-col gap-3 mb-4">
              {qaHistory.map((entry, i) => (
                <div key={i}>
                  ...
                  <div className="flex justify-start">
                    <div className="rounded-2xl px-4 py-2 text-sm max-w-[80%] leading-relaxed"
                      style={{ background: s.card, border: `1px solid ${s.border}` }}>
                      {entry.a}
                    </div>
                  </div>
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="rounded-2xl px-4 py-2 text-sm opacity-50"
                    style={{ background: s.card, border: `1px solid ${s.border}` }}>
                    思考中...
                  </div>
                </div>
